...
def check_job_status(request):
    ...
    if job_status == 'finished' or job_status.startswith("warning"):
        ...
        for i in os.listdir(f'/data/jobs/{job_id}/'):
            if '.out' in i:
                out_file_path = os.path.join('/data/jobs', job_id, i)
                with open(out_file_path, 'r') as checks:
                    lines = checks.readlines()[-1]
                    if any(x in lines for x in ['calculating', 'Running', 'Preparing', 'Generating']):
                        job_status_msg = lines
                break
        return JsonResponse({'status': job_status_msg, 'message': 'At the end of the process the results will be displayed.'})
...
def upload_file(request):
    if request.method == 'POST':
        input_option = request.POST.get('inputOption')  # "upload" or "uniprot"
        ...
        if input_option == 'upload':
            if not uploaded_file:
                return JsonResponse({'error': 'Missing main MSA file (.a3m, .fasta, etc.)'}, status=400)
...
def handle_upload(request, main_file=None, pdb_file=None, mutants_file=None, uniprot_id=None):
    try:
        ...
        if pdb_file:
            ...
            with open(pdb_file_path, 'wb+') as dest:
...
def results_view(request):
    ...
    ## drosophila db
        ...
        mapping_df['Gene_symbol'] = mapping_df['Gene_symbol'].str.lower()
    ...
    if df_snps is not None:
        ...
        # CASE 2 — "job*" mutations submitted by the user: highlight up to 20 phenotypes with distinct colors
            ...
            color_list = [
                ...
                "#8c564b", "#e377c2", "#ff1493", "#bcbd22", "#17becf",
    ...
    if os.path.exists(f'{data_path}{id_folder}/14.{prot_id}_GEMME_pLDDT.csv'):
        ...
        df_sefPrep = pd.read_csv(f'{data_path}{id_folder}/14.{prot_id}_GEMME_pLDDT.csv')
        ...
        fig_Seg.update_layout(
            ...
            paper_bgcolor="white",
            xaxis2=dict(title=dict(text="Residue", font=dict(size=15)),tickfont=dict(size=14),),
            yaxis2=dict(title=dict(text="Mutational sensitivity", font=dict(size=15)),range=[0, 1],tickfont=dict(size=14),),
            hovermode="x unified"
        )
