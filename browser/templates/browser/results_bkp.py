...
<noscript><iframe src="https://www.googletagmanager.com/ns.html?id=GTM-NT9G797M"
    ...
    <!-- End Google Tag Manager (noscript) -->
            ...
            <div class="collapse navbar-collapse justify-content-between" id="navbarNav">
              <ul class="navbar-nav ml-auto" id="navbarItems">
                ...
                <li class="nav-item" id="nav-past-jobs">
                    <a class="nav-link text-white" href="/drosophiladb">DrosophilaDB</a>
                </li>
                <li class="nav-item" id="nav-past-jobs">
      ...
      <div class="content" style="padding-bottom: 1cm;">
        <div class="container-fluid" style="width:95%;">

        <div class="form-row py-4">
            <div class="form-group col-md-10">
            <h5>Here are ProteoCast results for <b>{{ prot_name }}</b>. If you need help in exploring and interpreting them, please go to our <a href="/documentation" target="_blank" rel="noopener noreferrer">documentation</a>.<br>
            {% if query|slice:":4" == "2025" %}
            Your job (ID: <code class="code"> {{ query }}</code>) has used the MSA file <code class="code">{{ msa_file_job }}</code>
            {% if pdb_file and pdb_file != ".pdb" %} and the PDB file <code class="code">{{pdb_file}}</code>{% endif %}.
            {% endif %}
        </h5>
            {% if warning_message %}
        ...
        <h5 style="margin: 0; margin-right: 12px;">Share your feedback on ProteoCast</h5>
                <a href="https://docs.google.com/forms/d/e/1FAIpQLSdKJqiWffkf_vBx84mxuNKlcodhfauUtwpqLeDOhaboF7QEhg/viewform" 
                target="_blank" 
                rel="noopener noreferrer">
                    <button class="btn btn-primary example-button">Open Feedback Form</button>
                </a></div>
            </div>
        ...
        <div class="form-row" style="display: flex; align-items: flex-start;">
            <div class="heatmap" style="flex: 1; autosize: true;">
                ...
                <div class="buttons" style="margin-left: 20px;">
                   {% if heatmap_html %}
                   <label style="display: inline-block; margin-right: 20px;">
                       <input type="radio" name="heatmapToggle" value="heatmap_html" {% if not heatmapClasses_html and not heatmapRSA_html %}checked{% endif %}> 
                          RAW SCORES
                   </label>
                   {% endif %}
                   ...
                   <label style="display: inline-block; margin-right: 20px;">
                       <input type="radio" name="heatmapToggle" value="heatmapClasses_html" {% if not heatmap_html and not heatmapRSA_html %}checked{% endif %}>
                          VARIANT CLASSES
                   </label>
                   {% endif %}
                   {% if heatmapRSA_html %}
                   <label style="display: inline-block; margin-right: 20px;">
                      <input type="radio" name="heatmapToggle" value="heatmapRSA_html" {% if not heatmap_html and not heatmapClasses_html %}checked{% endif %}>
                ...
                <div class="heatmap-container">  
                    <!-- style="max-height: 600px; overflow: hidden; border: 1px solid #ddd; padding: 10px;">-->

                    {% if heatmap_html %}
                    <div id="heatmap_html" style="display: block; max-width: 100%;">
                        {{ heatmap_html|safe }}
                    </div>
                    {% endif %}
                    {% if heatmapRSA_html %}
                    <div id="heatmapRSA_html" style="display: none; max-width: 100%;">
                        {{ heatmapRSA_html|safe }}
                    </div>
                    {% endif %}
                    {% if heatmapClasses_html %}
                    <div id="heatmapClasses_html" style="display: none; max-width: 100%;">
                        {{ heatmapClasses_html|safe }}
                    </div>
                    {% endif %}
                    {% if heatmapSNPs_html %}
                    <div id="heatmapSNPs_html" style="display: none; max-width: 100%;">
                        {{ heatmapSNPs_html|safe }}
                    </div>
                    {% endif %}
                    {% if not heatmap_html and not heatmapClasses_html and not heatmapSNPs_html and not heatmapRSA_html %}
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                document.querySelectorAll('input[name="heatmapToggle"]').forEach(button => {
                    button.addEventListener('change', function() {
                       ...
                       const heatmapRSAHtmlDiv = document.getElementById('heatmapRSA_html');
                       ...
                       if (this.value === 'heatmap_html') {
                           setDisplay(heatmapHtmlDiv, 'block');
                           setDisplay(heatmapClassesHtmlDiv, 'none');
                           setDisplay(heatmapSNPsHtmlDiv, 'none');
                           setDisplay(heatmapRSAHtmlDiv, 'none');
                       } else if (this.value === 'heatmapClasses_html') {
                           setDisplay(heatmapHtmlDiv, 'none');
                           setDisplay(heatmapClassesHtmlDiv, 'block');
                           setDisplay(heatmapSNPsHtmlDiv, 'none');
                           setDisplay(heatmapRSAHtmlDiv, 'none');
                       } else if (this.value === 'heatmapSNPs_html') {
                           setDisplay(heatmapHtmlDiv, 'none');
                           setDisplay(heatmapClassesHtmlDiv, 'none');
                           setDisplay(heatmapSNPsHtmlDiv, 'block');
                           setDisplay(heatmapRSAHtmlDiv, 'none');
                       } else if (this.value === 'heatmapRSA_html') {
                           setDisplay(heatmapHtmlDiv, 'none');
                           setDisplay(heatmapClassesHtmlDiv, 'none');
                           setDisplay(heatmapSNPsHtmlDiv, 'none');
                           setDisplay(heatmapRSAHtmlDiv, 'block');
                       }
        ...
        <div class="form-row">
            <div class="form-group col">
                <h2 style="margin-top: 20px;">Input Alignment</h2>
            </div>
            <div class="form-group col px-4">
                <h2 style="margin-top: 20px;">Raw Score Distribution</h2>
            </div>
        </div>
        <div class="form-row">
            <div class="form-group col">
        ...
        <div class="form-row" style="display: flex; align-items: flex-start;"> 
            ...
            <p> 
                Mutational sensitivity as estimated by GEMME average score per residue. Each delineated segment can be considered as displaying a homogeneous mean sensitivity. The identified segments are colored red if their mean value (horizontal line) is higher than one neighboring segment, and purple if it is higher than both neighboring segments.
                {% if pdb_file and pdb_file != ".pdb" %}
                The pLDDT confidence score from the AlphaFold2-predicted 3D structure is shown as a color bar on top. Orange and yellow indicate very low to low confidence, while light and dark blue represent high to very high confidence regions.
                {% endif %} 
            </p>
            <div class="heatmap-container">
            {% if fig_segmentation %}
            <div id="fig_segmentation" style="display: block; max-width: 100%;">
        ...
        <!-- 3D structure -->
        <div class="form-row">
            <h2 style="margin-top: 30px;">3D Structure</h2>
        ...
        <div class="form-row" style="min-height: 600px;">
            ...
            <div class="form-group col-2 pl-5 py-4">
                <p style="margin-top: 20px;"> COLOR BY: </p>
                <div class="buttons">
                    <label style="display: inline-block;">
    ...
    <footer class="bg-light text-center py-3">
            <p class="mb-0">ProteoCast - This website is free and open to all users and there is no login requirement. <br> This website uses Google Analytics cookies for the sole purpose of monitoring usage.</p>
    </footer>
    <script>
        document.addEventListener('DOMContentLoaded', function () {
